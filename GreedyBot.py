...
while True:
    ...
    for i in range(0, len(ships)):
        ...
        if target_planet:
            ...
            # all nonenemy planets are full, so go for closest enemy planet
            # that's open (doesn't really make sense rn)
            ...
            if target_planet:
                ...
                if ship.can_dock(target_planet):
                    ...
                    navigate_command = ship.navigate(ship.closest_point_to(target_planet,
                        min_distance=0), game_map,
                        speed=int(hlt.constants.MAX_SPEED))
                    if navigate_command:
                        command_queue.append(navigate_command)
            else:
                # all enemy planets are full too, just go ham on them
                if len(enemy_planets) > 0:
                    target_planet = enemy_planets[i % len(enemy_planets)]
                    navigate_command = ship.navigate(ship.closest_point_to(target_planet,
                        min_distance=0), game_map,
                        speed=int(hlt.constants.MAX_SPEED))
                    if navigate_command:
